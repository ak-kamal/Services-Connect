      ...
      <div className="min-h-screen bg-base-200 flex flex-col">
        ...
        <div className="flex justify-center items-center h-64">
          <div className="text-center">
            ...
            <p className="mt-2 text-gray-500">{t('provider.stats.loading')}</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-base-200 flex flex-col">
      ...
      <div className="p-6 max-w-7xl mx-auto">
        <h2 className="text-2xl font-bold mb-6">{t('provider.stats.title')}</h2>
        
        {/* Stats Cards */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <div className="bg-green-500 text-white p-6 rounded-lg shadow">
            <h3 className="text-sm font-semibold">{t('provider.stats.totalEarnings')}</h3>
            <p className="text-3xl font-bold mt-2">{stats.totalEarnings.toFixed(2)} BDT</p>
          </div>
          
          <div className="bg-blue-500 text-white p-6 rounded-lg shadow">
            <h3 className="text-sm font-semibold">{t('provider.stats.totalJobs')}</h3>
            <p className="text-3xl font-bold mt-2">{stats.totalJobs}</p>
          </div>
          
          <div className="bg-purple-500 text-white p-6 rounded-lg shadow">
            <h3 className="text-sm font-semibold">{t('provider.stats.avgDistance')}</h3>
        ...
        <div className="bg-white p-6 rounded-lg shadow mb-8">
          <h4 className="text-xl font-bold mb-2">{t('provider.stats.ratingProgression')}</h4>
          <p className="text-sm text-gray-500 mb-4">{t('provider.stats.ratingProgressionDesc')}</p>
          {stats.ratingProgression.length > 0 ? (
            <ResponsiveContainer width="100%" height={400}>
              <LineChart data={stats.ratingProgression}>
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis dataKey="jobNumber" label={{ value: t('provider.stats.jobNumber'), position: 'insideBottom', offset: -5 }} />
                <YAxis domain={[0, 5]} label={{ value: t('provider.stats.averageRating'), angle: -90, position: 'insideLeft' }} />
                <Tooltip formatter={(value) => [`${value} ⭐`, t('provider.stats.averageRating')]} labelFormatter={(label) => `${t('provider.stats.job')} #${label}`} />
                <Legend />
                <Line type="monotone" dataKey="rating" stroke="#8884d8" name={t('provider.stats.averageRating')} strokeWidth={2} dot={{ r: 4 }} />
              </LineChart>
            </ResponsiveContainer>
          ) : (
            <div className="text-center py-12 bg-gray-50 rounded-lg">
              <p className="text-gray-500">{t('provider.stats.noRatingData')}</p>
            </div>
          )}
        </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow mb-8">
          <h4 className="text-xl font-bold mb-2">{t('provider.stats.earningsTrend')}</h4>
          <p className="text-sm text-gray-500 mb-4">{t('provider.stats.earningsTrendDesc')}</p>
          {stats.earningsTrend.length > 0 ? (
            <ResponsiveContainer width="100%" height={400}>
              <BarChart data={stats.earningsTrend}>
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis dataKey="monthName" label={{ value: t('provider.stats.month'), position: 'insideBottom', offset: -5 }} />
                <YAxis label={{ value: t('provider.stats.earningsBDT'), angle: -90, position: 'insideLeft' }} />
                <Tooltip formatter={(value) => [`${value} BDT`, t('provider.stats.earnings')]} labelFormatter={(label) => `${label}`} />
                <Legend />
                <Bar dataKey="totalEarnings" fill="#A7C7E7" name={t('provider.stats.totalEarnings')} />
              </BarChart>
            </ResponsiveContainer>
          ) : (
            <div className="text-center py-12 bg-gray-50 rounded-lg">
              <p className="text-gray-500">{t('provider.stats.noEarningsData')}</p>
            </div>
          )}
        </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow mb-8">
          <h4 className="text-xl font-bold mb-2">{t('provider.stats.categoryBreakdown')}</h4>
          <p className="text-sm text-gray-500 mb-4">{t('provider.stats.categoryBreakdownDesc')}</p>
          
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Left Chart: Average Rating by Category */}
            <div>
              <h5 className="text-lg font-semibold mb-3 text-center">{t('provider.stats.avgRatingByCategory')}</h5>
              {stats.categoryAvgRating.length > 0 ? (
                <ResponsiveContainer width="100%" height={400}>
                  <BarChart data={stats.categoryAvgRating}>
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis dataKey="_id" label={{ value: t('provider.category'), position: 'insideBottom', offset: -5 }} />
                    <YAxis domain={[0, 5]} label={{ value: t('provider.stats.averageRating'), angle: -90, position: 'insideLeft' }} />
                    <Tooltip formatter={(value) => [`${value.toFixed(2)} ⭐`, t('provider.stats.averageRating')]} labelFormatter={(label) => `${label}`} />
                    <Legend />
                    <Bar dataKey="averageRating" fill="#8884d8" name={t('provider.stats.averageRating')} />
                  </BarChart>
                </ResponsiveContainer>
              ) : (
                <div className="text-center py-12 bg-gray-50 rounded-lg h-[400px] flex items-center justify-center">
                  <p className="text-gray-500">{t('provider.stats.noCategoryRatingData')}</p>
                </div>
              )}
            </div>
            ...
            <div>
              <h5 className="text-lg font-semibold mb-3 text-center">{t('provider.stats.jobsByCategory')}</h5>
              {stats.categoryBreakdown.length > 0 ? (
                <ResponsiveContainer width="100%" height={400}>
                  <BarChart data={stats.categoryBreakdown}>
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis dataKey="_id" label={{ value: t('provider.category'), position: 'insideBottom', offset: -5 }} />
                    <YAxis label={{ value: t('provider.stats.numberOfJobs'), angle: -90, position: 'insideLeft' }} />
                    <Tooltip formatter={(value) => [`${value} ${t('provider.stats.jobs')}`, t('provider.stats.jobsCompleted')]} labelFormatter={(label) => `${label}`} />
                    <Legend />
                    <Bar dataKey="jobsCompleted" fill="#82ca9d" name={t('provider.stats.jobsCompleted')} />
                  </BarChart>
                </ResponsiveContainer>
              ) : (
                <div className="text-center py-12 bg-gray-50 rounded-lg h-[400px] flex items-center justify-center">
                  <p className="text-gray-500">{t('provider.stats.noCategoryData')}</p>
                </div>
              )}
            </div>
          </div>
